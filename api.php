...
function removeEmoji(string|null $text): string|null
{
...
function addParamToArray(&$array, $param, $type = null): void
{
    global $PARAMS;
    if (!isset($PARAMS[$param])) {
        return;
...
function utflen($str): int
{
    return mb_strlen($str, 'utf-8');
}

function utfsubstr(string $s, $offset, $length = null): string
{
...
try {
    ...
    checkParamEmpty('v');
    $v = (int) $PARAMS['v'];
    if ($v < MIN_API_VERSION || $v > API_VERSION) {
        ...
        if (count($folders) > 0) {
            ...
            foreach ($folders as $f) {
                if (($f['_'] ?? '') == 'dialogFilterDefault' || !isset($f['id'])) {
                    ...
                    $res['res'][] = ['id' => $f['id'], 't' => $f['title']['text'] ?? $f['title']];
                }
            }
        }
        json($res);
    ...
    case 'getContacts':
        ...
        foreach ($rawData['contacts'] as $contact) {
            $res[] = parseUser(findPeer(getId($contact), $rawData));
        }
        json(['res' => $res]);
        break;
    case 'joinChannel':
    ...
    case 'searchChats':
        ...
        foreach ($rawData['my_results'] as $c) {
            ...
            $res[] = $r['id'] < 0 ? parseChat($r) : parseUser($r);
        }
        ...
        foreach ($updates as $update) {
            ...
            if ($type == 'updateNewMessage' || $type == 'updateNewChannelMessage') {
                ...
                if ($peers && $type == 'updateNewChannelMessage') {
                    if (!in_array(strval(getId($msg['peer_id'])), $peers))
                        continue;
                } else {
                    $info = $MP->getFullInfo($msg['peer_id']);
                    if ($info['Chat']['left'] ?? false)
    ...
    case 'getPeerInfo':
        ...
        if (isset($raw['User'])) {
            ...
            if (($raw['full']['participants_count'] ?? $chat['participants_count'] ?? 0) > 0) {
                $r['count'] = $raw['full']['participants_count'] ?? $chat['participants_count'];
            }
